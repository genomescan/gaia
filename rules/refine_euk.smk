rule acr_refine:
    container:
        CONTAINERS["euk"]
    input:
        bins_dir="outputs/{sample}/reports/binning/all_bins",
        coverage="outputs/{sample}/reports/mapping_depth/{sample}.depth.txt",
        bins_done="outputs/{sample}/reports/binning/all_bins/.done",
        acr_db_dir=config["refinement"]["euk"]["acr_params"]["db_path"]
    output:
        refined_dir=directory("outputs/{sample}/reports/refinement/euk/acr/refined_bins"),
        done="outputs/{sample}/reports/refinement/euk/acr/.done"
    threads:
        P["threads"].get("acr", 16)
    params:
        outdir="outputs/{sample}/reports/refinement/euk/acr",
        prefix=ACR_PREFIX,
...
rule keep_euk_from_acr:
    input:
        refined_dir="outputs/{sample}/reports/refinement/euk/acr/refined_bins",
        acr_done="outputs/{sample}/reports/refinement/euk/acr/.done"
    output:
...
rule drep_euk_compare:
    container:
        CONTAINERS["euk"]
    input:
        euk_done="outputs/{sample}/reports/refinement/euk/euk_bins/.done",
        euk_dir="outputs/{sample}/reports/refinement/euk/euk_bins"
    output:
        cluster_info="outputs/{sample}/reports/refinement/euk/drep/data_tables/Cdb.csv",
        done="outputs/{sample}/reports/refinement/euk/drep/.done"
    threads:
        P["threads"].get("drep", 16)
    params:
        outdir="outputs/{sample}/reports/refinement/euk/drep",
        extra=DREP_EXTRA,
...
rule eukcc_euk_candidates:
    ...
    input:
        ...
        euk_dir="outputs/{sample}/reports/refinement/euk/euk_bins"
    output:
        summary="outputs/{sample}/reports/refinement/euk/eukcc/eukcc.csv",
        done="outputs/{sample}/reports/refinement/euk/eukcc/.done"
    threads:
        P["threads"].get("eukcc", 16)
    params:
        outdir="outputs/{sample}/reports/refinement/euk/eukcc",
        db=EUKCC_DB,
        extra=EUKCC_EXTRA,
...
rule select_best_euk_bins:
    input:
        ...
        cluster_info="outputs/{sample}/reports/refinement/euk/drep/data_tables/Cdb.csv",
        drep_done="outputs/{sample}/reports/refinement/euk/drep/.done",
        eukcc="outputs/{sample}/reports/refinement/euk/eukcc/eukcc.csv",
        eukcc_done="outputs/{sample}/reports/refinement/euk/eukcc/.done"
    output:
...
rule bat_classify:
    container:
        CONTAINERS["euk"]
    input:
        final_done="outputs/{sample}/reports/refinement/euk/final_bins/.done",
        bins_dir="outputs/{sample}/reports/refinement/euk/final_bins/selected_bins"
    output:
        classification="outputs/{sample}/reports/refinement/euk/bat/bin2classification.txt",
        done="outputs/{sample}/reports/refinement/euk/bat/.done"
    threads:
        P["threads"].get("bat", 24)
    params:
        outdir="outputs/{sample}/reports/refinement/euk/bat",
        db=BAT_DB,
